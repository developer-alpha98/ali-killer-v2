...
sprite.src = 'img/hidden-game.png';
...
//Game
function game() {
    record = localStorage.getItem("highScore") ? parseInt(localStorage.getItem("highScore")) : 0;
        ...
        if(playing) {
            ...
            if(gameOver) {
                ...
                if (dist < 27) {
                    if(e.type == 'click') {
                        playing = true;
                        document.getElementById("closeGameBtn")?.classList.remove("display-none-class");
    ...
    function planet() {
        ...
        // فقط تصویر سیاره رسم میشه، نه دایره سفید
        ctx.translate(cW/2,cH/2);
    ...
    function _asteroids() {
        ...
        for(var i = 0; i < asteroids.length; i++) {
            if (!asteroids[i].destroyed) {
                ...
                ctx.drawImage(
                    ...
                    asteroids[i].moveY += 0.7/(asteroids[i].size),
        ...
        if(asteroids.length - destroyed < 8 + (Math.floor(destroyed/6))) {
            newAsteroid();
        }
    }
    ...
    function start() {
        if(!gameOver) {
            ...
            if(playing) {
                ...
                ctx.font = "20px yekanbakh";
                ctx.fillStyle = "white";
                ctx.textBaseline = 'middle';
                ctx.textAlign = "left";
                ctx.fillText('رکورد: '+record+'', 20, 30);

                ctx.font = "40px yekanbakh";
                ctx.fillStyle = "white";
            ...
            ctx.font = "60px yekanbakh";
            ctx.fillStyle = "white";
            ctx.textAlign = "center";
            ctx.fillText("ای بابا باختی که",cW/2,cH/2 - 150);

            ctx.font = "20px yekanbakh";
            ctx.fillStyle = "white";
            ctx.textAlign = "center";
            ctx.fillText("چندتا بالن زدی: "+ destroyed, cW/2,cH/2 + 140);
            ...
            ctx.fillStyle = "white";
            ctx.textAlign = "center";
            ctx.fillText("رکورد: "+ record, cW/2,cH/2 + 185);
...
document.getElementById("closeGameBtn").addEventListener("click", () => {
    ...
    canvasEl?.classList.add("display-none-class");
    closeBtn?.classList.add("display-none-class");

    document.querySelector(".header")?.classList.remove("display-none-class");
    document.querySelector(".main")?.classList.remove("display-none-class");
    document.querySelector(".footer")?.classList.remove("display-none-class");
});
